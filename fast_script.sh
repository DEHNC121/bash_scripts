...
# Function to checkout to master in Git repository if current branch is dev-denys-rolling-rac
perform_git_operations() {
    ...
    if [ "$branch_name" == "dev-denys-rolling-rac" ]
    then
        ...
        repo_output+=$(print_info "Already on master branch or not on dev-denys-rolling-rac branch")
    fi
